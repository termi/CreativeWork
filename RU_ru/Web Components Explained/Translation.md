 ...
 * шаблоны ([templates](http://dvcs.w3.org/hg/webcomponents/raw-file/tip/explainer/index.html#template-section)), определяют кусок неактивной разметки, которая может быть использована в дальнейшем;
 * декораторы ([decorators](http://dvcs.w3.org/hg/webcomponents/raw-file/tip/explainer/index.html#decorator-section)), позволяют через CSS управлять визуальными и поведенческими изменениями в шаблонах;
 ...
 * теневой DOM ([shadow DOM](http://dvcs.w3.org/hg/webcomponents/raw-file/tip/explainer/index.html#shadow-dom-section)), определяет, как представление и поведение _декораторов_ и _пользовательские элементы_ сочетаются друг с другом в дереве DOM.
...
Например, разработчику может понадобиться определить DOM-структуру, которая создается несколько раз в документе, а затем создать его экземпляр когда это необходимо.
    ...
    // Заполнить содержание и img[src] значения в шаблоне.
    // …
...
Элемент [`<content>`](http://dvcs.w3.org/hg/webcomponents/raw-file/tip/spec/shadow/index.html#content-element) указывает на место, куда декоратор (точнее, его содержимое) должно быть вставлено.
