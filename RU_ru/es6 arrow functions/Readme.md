...
Компиляция из вольного перевода статьи [Understanding ECMAScript 6 arrow functions](http://www.nczonline.net/blog/2013/09/10/understanding-ecmascript-6-arrow-functions/)
...
(их мы не разбираем в рамках данной статьи - вы можете почитать про них в статье
[http://habrahabr.ru/post/175371/](Обзор ECMAScript 6, следующей версии JavaScript), хотя информация из этой статьи
частично устарела).
Пример:
```javascript
...
```
В приведённом коже, объект `pageHandler` должен обрабатывать клики на странице. Метод `init()` навешивает обработчик
...
```
Использование таких методов как массивные `sort()`, `map()`, `reduce()` и т.д., может быть упрошено с использованием
